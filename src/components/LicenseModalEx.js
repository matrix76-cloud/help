...
import SignModalEx from './SignModalEx';
...
export default function LicenseModalEx({callback, data}) {
  const [open, setOpen] = React.useState(true);

  const [name, setName] = React.useState('');
  const [refresh, setRefresh] = React.useState('');
  const [signmove, setSignmove] = React.useState(false);
  ...
  return (
    <div>
      {
        signmove == true && <SignModalEx callback={signmodalcallback} />
      }
       ...
       <Box sx={style}>
            ...
            <ApplyItem >
                <div style={{dispaly:"flex", alignItems:"flex-end", marginRight :35, justifyContent:"center"}}>   
                    <FilterApplyButton onClick ={_handlelicenseapply}><FilterApplyButtonText>계약 내용을 동의이해 하였고 이에 서명합니다</FilterApplyButtonText></FilterApplyButton>
                </div>
            </ApplyItem>
            </Box>
