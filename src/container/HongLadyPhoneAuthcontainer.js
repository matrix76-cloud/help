...
const HongLadyPhoneAuthcontainer = ({role}) => {
    ...
    {loading == true ? (<Loading containerStyle={{ marginTop: 200 }}
     />) :
      <div style={{marginTop:50}}>
        <BoxLayer height={650}>
          <div>홍여사로 신원인증을 하기 위해서는 핸드폰 인증을 통한 본인 확인절차을 받아야 합니다</div>

          <PictureBoxLayer>
                <div>핸드폰 인증 화면(네이버 클라우드 이용)</div>
          </PictureBoxLayer>


          <Button
            buttonText={"핸드폰 본인 인증이 정상적으로 진행 되었습니다"}
...
export default React.memo(HongLadyPhoneAuthcontainer);
